...
switch ($type) {
    case 'add':
        if (!Cart::getProductCart($db, $productId, $session->getId())) {
            if (Cart::addToCart($db, $session->getId(), $productId)) {
                $response["status"] = "success";
                $response["message"] = "Product Added Successfully to Cart!";
                $response["type"] = "Add";
            } else {
            ...
            if(Cart::removeProduct($db, $productId, $session->getId())){
                $response["status"] = "success";
